...
## Common DEGs among the 3 ages
M3 <- read.csv("3months_male_APOE4vs3.csv", header=T)
M12 <- read.csv("12months_male_APOE4vs3.csv", header=T)
M24 <- read.csv("24months_male_APOE4vs3.csv", header=T)
  ##I need to make vectors from the gene symbols in the three results datasets
list3M <- as.list( M3[,2], drop=false) 
list12M <- as.list( M12[,2], drop=false) 
list24M <- as.list( M24[,2], drop=false) 
## Calculate which genes overlap in the three DEGs files
common_elements <- Reduce(dplyr::intersect, list(M3[, 2], M12[, 2], M24[, 2]))
overlap <- as.data.frame(common_elements)
  ## Export table with the common genes
write.csv(overlap,"common_genes_ages.csv")
